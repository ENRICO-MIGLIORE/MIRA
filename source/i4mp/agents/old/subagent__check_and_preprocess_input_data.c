...
int subagent__check_and_preprocess_input_data (const char* calling_function_name, const char* data, size_t data_len, char** tokens, size_t* tokens_len, size_t tokens_max, size_t* token_count, size_t minimum_token_count)
{
	...
	size_t i;
	size_t seps_len[SUBAGENT__FIELDS_SEPARATORS_MAX];
	size_t seps_count;
	size_t tok_count;
	const char* fn;
	const char* seps[SUBAGENT__FIELDS_SEPARATORS_MAX] = SUBAGENT__FIELDS_SEPARATORS;
	

	fn = "agent__preprocess_input_data()";
	...
	for (i = 0; i < SUBAGENT__FIELDS_SEPARATORS_MAX; i++)
	{
		seps_len[i] = strlen(seps[i]);
	}


	seps_count = SUBAGENT__FIELDS_SEPARATORS_MAX;
	...
	rc = utils__libc__trim_tokens(&tokens[0], tokens_len, tok_count, SUBAGENT__CHARACTER_TO_TRIM);
	if (rc != 0)
	{
	...
	escape_sequence = SUBAGENT__UNESCAPE_SEQUENCE;
	escape_sequence_len = SUBAGENT__UNESCAPE_SEQUENCE_LEN;
	c = SUBAGENT__UNESCAPE_REPLACEMENT_CHAR;
