...
int subagent__get_machine_connection_status (subagent_machine_t* machine, char* buffer, size_t buffer_size, size_t* buffer_len)
{
	...
	if ((rc == 0) && (machine->connection_status_valid == 1))
	{
		if (machine->connection_status == 0)
		{
			buffer[0] = '0';   /* i4_agency not connected to remote machine */
		}
		else
		{
			buffer[0] = '1';   /* i4_agency connected to remote machine */
		}
	}
	else
	{
