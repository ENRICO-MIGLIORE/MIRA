...
int siemens_open62541__set_name (int md, char* buffer, size_t buffer_len)
{
	char* buf;
	size_t buffer_size;
	int rc;
	...
	buf = so->machine[md].config.name;
	buffer_size = sizeof(so->machine[md].config.name) - 1;  /* -1 is the room we need to safely terminate the string */
