...
static char* fn = "fanuc_focas__private__load_application_configuration()";
...
int fanuc_focas__private__load_application_configuration (size_t *section_count)
{
	...
	/*
	 * Set section data of the agent:  [FANUC-FOCAS AGENT]
	 */
	section.name = ff->agent_section_name;
	section.function = found_agent_section;
	application_names_values_max = sizeof(names_values) / sizeof(utils__sdb__value_callback_t);


	/*
	 * extract data from config file or buffer looking for section: ff->agent_section_name
	 */
	start_record = 1;

	if (ff->config_mode == 0)
	{
		rc = utils__sdb__count_sections_from_file(&ff->sdb, ff->agent_configuration_file_name, ff->agent_section_name, start_record, &sect_count, &end_of_file);
		if (rc != 0)
		{
	...
	{
		rc = utils__sdb__count_sections_from_buffer(&ff->sdb, ff->agent_configuration_file_name, ff->agent_section_name, start_record, &sect_count, &end_of_file);
		if (rc != 0)
		{
	...
	if (ff->config_mode == 0)
	{
		rc = utils__sdb__read_configuration_from_file(p, &ff->sdb, ff->agent_configuration_file_name, &section, names_values, application_names_values_max, start_record, &record_number, &end_of_file);
		if (rc != 0)
		{
	...
	{
		rc = utils__sdb__read_configuration_from_buffer(p, &ff->sdb, ff->agent_configuration_file_name, &section, names_values, application_names_values_max, start_record, &record_number, &end_of_file);
		if (rc != 0)
		{
...
static int enable_log_file (const char* value, size_t value_len)
{
	...
	data = &ff->config.enable_log_file;
...
static int set_log_file (const char* value, size_t value_len)
{
	...
	buffer = ff->config.log_file_name;
	buffer_size = sizeof(ff->config.log_file_name);
...
static int set_max_log_file_lines_per_minute (const char* value, size_t value_len)
{
	...
	data = &ff->config.max_log_file_lines_per_minute;
	...
	if (ff->config.max_log_file_lines_per_minute == 0)
	{
		return -200;
	}
...
static int enable_table_layout_output (const char* value, size_t value_len)
{
	...
	data = &ff->config.enable_table_layout_output;
...
static int enable_application_name_on_output (const char* value, size_t value_len)
{
	...
	data = &ff->config.enable_application_name_on_output;
...
static int set_application_name (const char* value, size_t value_len)
{
	...
	buffer = ff->config.application_name;
	buffer_size = sizeof(ff->config.application_name);
...
static int set_thread_sleep_time_seconds(const char* value, size_t value_len)
{
	...
	data = &ff->config.thread_sleep_time_seconds;
	...
	if (ff->config.thread_sleep_time_seconds == 0)
	{
		ff->config.thread_sleep_time_seconds = 5;
	}
...
static int enable_connection_monitor_task(const char* value, size_t value_len)
{
	...
	data = &ff->config.enable_connection_monitor_task;
...
static int set_probability_of_success_of_events_in_simulation_mode (const char* value, size_t value_len)
{
	...
	data = &ff->config.probability_of_success_of_events_in_simulation_mode_in_percentage;
	...
	if ((ff->config.probability_of_success_of_events_in_simulation_mode_in_percentage < 0) || (ff->config.probability_of_success_of_events_in_simulation_mode_in_percentage > 1))
	{
		return -200;
	}

	ff->probability_of_success_of_events_in_simulation_mode_in_percentage = (int) (ff->config.probability_of_success_of_events_in_simulation_mode_in_percentage * 1000);
