...
int okuma_mt__private__read_cycle_time__real (int md)
{
	...
	if (om->machine[md].cycle_time_fpc.valid != 1)
	{
		return -100;

	}
	
	om->machine[md].cycle_time = om->machine[md].cycle_time_fpc.cycle_time;

	sprintf(om->machine[md].cycle_time_utf_8, "%f", om->machine[md].cycle_time);
