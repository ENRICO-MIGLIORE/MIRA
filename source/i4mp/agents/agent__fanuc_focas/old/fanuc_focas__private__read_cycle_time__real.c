...
int fanuc_focas__private__read_cycle_time__real (int md)
{
	...
	if (ff->machine[md].cycle_time_fpc.valid != 1)
	{
		return -100;

	}
	
	ff->machine[md].cycle_time = ff->machine[md].cycle_time_fpc.cycle_time;

	sprintf(ff->machine[md].cycle_time_utf_8, "%f", ff->machine[md].cycle_time);
