...
void* agent__fanuc_focas__thread (void* arg)
{
	...
	while (machine->thread_stop_request == 0)
	{
		...
		agent__fanuc_focas__sleep_thread(machine);

		rc = agent__fanuc_focas__connect_to_machine(machine);
	...
	agent__fanuc_focas__suspend_thread(machine);
