...
int siemens_open62541__get_part_program_name (int md, char* buffer, size_t buffer_size, size_t* buffer_len)
{
	...
	else if (so->machine[md].part_program_name_valid != 1)
	{
		*buffer_len = 0;
	}
	else
	{
		strcpy(buffer, so->machine[md].part_program_name_utf_8);
		*buffer_len = so->machine[md].part_program_name_utf_8_len;
	}
