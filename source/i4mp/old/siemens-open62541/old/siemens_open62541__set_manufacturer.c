...
int siemens_open62541__set_manufacturer (int md, char* buffer, size_t buffer_len)
{
	...
	buf = so->machine[md].config.manufacturer;
	buffer_size = sizeof(so->machine[md].config.manufacturer) - 1;  /* -1 is the room we need to safely terminate the string */
