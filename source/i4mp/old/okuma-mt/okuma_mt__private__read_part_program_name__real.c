...
int okuma_mt__private__read_part_program_name__real(int md)
{
	...
	rc = utils__xml__extract_inner_xml(om->machine[md].receive_buffer, om->machine[md].receive_buffer_len, "Program", 7, om->machine[md].part_program_name_utf_8, sizeof(om->machine[md].part_program_name_utf_8), &om->machine[md].part_program_name_utf_8_len);
	if (rc != 0)
	{
		return -300;
	}
