...
int okuma_mt__get_production_status (int md, char* buffer, size_t buffer_size, size_t* buffer_len)
{
	...
	{
		buffer[0] = '3';  		/* production status not available because agent is not configured to connect to machine */
	}
	else if (om->machine[md].production_status_valid != 1)
	{
		buffer[0] = '2';  		/* production status not available */
	}
	else if (om->machine[md].production_status == 1)
	{
