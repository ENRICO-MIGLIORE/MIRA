...
unsigned int __stdcall fgg__machine_1__main_thread (void *arg)
{
	...
	while ((fgg.cncs->machine[0].thread_status[0] == 1) || (fgg.cncs->machine[0].thread_status[0] == 2))
	{
		...
		for (i = 0; i < UTIMAC_MAX; i++)
		{
			...
			case CNC_1__CONNECT:
				rc[i] = fgg__cnc__open62541_connect  (client3[i], &fgg.cncs->machine[index[i]].parameter[CNC_PARAMETER_IP_1], 
			...
			case CNC_1__RECONNECT:
				fgg__cnc__open62541_clear_client  (client3[i]);
			...
			case CNC_1__READY__STATE:
				...
				{ 
					//leggo i file da scp
					fgg__cnc__enca_pscp_get_program_files (&fgg.cncs->machine[index[0]].parameter[CNC_PARAMETER_IP_2],  
				...
				rc[i] |= fgg__cnc__open62541_get_machined_parts  (client3[i],  &fgg.cncs->machine[index[i]].parameter[CNC_PARAMETER_PERCORSO_PEZZI_ASSOLUTI], &pieces_machined[i]);

				rc[i] |= fgg__cnc__open62541_get_time_of_cycle  (client3[i],  &fgg.cncs->machine[index[i]].parameter[CNC_PARAMETER_PERCORSO_TEMPO_DI_CICLO], &cycle_time[i]);

				rc[i] |= fgg__cnc__open62541_number_of_alarm  (client3[i], &fgg.cncs->machine[index[i]].parameter[CNC_PARAMETER_PERCORSO_ALLARMI], &alarm[i]);

				rc[i] |= fgg__cnc__open62541_name_program_in_execution  (client3[i], &fgg.cncs->machine[index[i]].parameter[CNC_PARAMETER_NOME_PROG_IN_ESECUZIONE], &prog[i]); 

				rc[i] |= fgg__cnc__open62541_get_state_machine  (client3[i], &fgg.cncs->machine[index[i]].parameter[CNC_PARAMETER_STATO_MACCHINA], &state_machine[i]); 

				rc[i] |= fgg__cnc__open62541_get_avanzamento_barra (client3[i], &fgg.cncs->machine[index[i]].parameter[CNC_PARAMETER_AVANZAMENTO_BARRA], &avanzamento[i]); 

				rc[i] |= fgg__cnc__open62541_get_emergency  (client3[i], &fgg.cncs->machine[index[i]].parameter[CNC_PARAMETER_FUNGO_EMERGENZA], &state_em);
