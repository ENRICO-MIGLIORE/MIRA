...
int lathe_machine__get_cycle_time (lathe_machines_agent_t * proxy, size_t machine_id, char* buffer, size_t buffer_size, size_t* buffer_len)
{
	...
	if (proxy->lathe_machine[fd].cycle_time_valid != 1)
	{
		pthread_mutex_unlock(&proxy->pthread_mutex);
		return -800;
	}

	strcpy(buffer, proxy->lathe_machine[fd].cycle_time_utf_8);
	*buffer_len = proxy->lathe_machine[fd].cycle_time_utf_8_len;
