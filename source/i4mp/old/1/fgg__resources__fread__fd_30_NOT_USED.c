...
*/
static char *fn = "fgg__resources__fread__fd_30()";



unsigned long fgg__resources__fread__fd_30 (void *buffer, unsigned long buffer_size)
{
	...
	char buffer_tmp[FGG__CONFIG__STACK_BUFFER_SIZE];
	unsigned long lang;
	...
	{
		p[0] = '0';
		fgg__resources.config.resource[30].eof = 1;
		fgg__resources.last_error_fd = 30;
	...
	value_len[0] = sprintf (buffer_tmp,"%d",count);
	value[0] = buffer_tmp;
	...
	for (i = 0; i < count; i++)
	{
		...
		if (fgg__resources.data.value[FGG__PO__FIELD__JOB_STATUS][0] == '0')
		{
			//se FGG__PO__JOB_STATUS__V1 == 0 vuol dire che la commessa e' aperta e allora la pubblico
