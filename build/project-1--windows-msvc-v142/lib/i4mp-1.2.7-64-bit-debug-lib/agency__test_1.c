...
#define TEST_1__CONFIG_FILE_NAME       "D:\\PROGS\\prog-97--i4mp\\deployment-examples\\project-1--windows\\example-1\\i4mp_config.txt"
...
int agency__test_1 (const char* config_file_name, const char* log_file_name, int enable_printf, int enable_log_file, int enable_table_layout_output, int test_cycles)
{
	...
	config.file_name = TEST_1__CONFIG_FILE_NAME;
	...
	rc = agency__get_machine_id(md, buffer, sizeof(buffer), &buffer_len);
	if (rc != 0)
	{
		return -500;
	}

	rc = agency__get_machine_name(md, buffer, sizeof(buffer), &buffer_len);
